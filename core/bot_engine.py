...
class BotEngine(threading.Thread):
    ...
    def reload_runtime(self):
        ...
        self._patrol_panel_cache = None   # ★ 让 _draw_patrol 下次重建合成图
        self._th_cache = None
    ...
    def _tick(self):
        ...
        ★ v24 修改点
        ============================================================
        ① 帧内阈值缓存：_th_frame_cache 每帧开头重置
           → 一帧内多次调 _thresholds() 只算一次
           → 跨帧实时读 cfg，GUI 滑块改动立刻生效
              （原来跨帧缓存导致"攻击范围写死"）

        ② 血蓝条 round-robin：每帧只算 1 个 bar（HP/MP/EXP 轮着来）
           → 单帧峰值从 6~15ms 降到 2~5ms
           → 每个 bar 每 3 帧刷新一次（10Hz），喝药完全够
              （原来每 5 帧算 3 个，单帧尖峰大）
        ============================================================
        """
        _T = {}
        self._t0 = time.time()
        self._th_frame_cache = None   # ★ 每帧重置 → 跨帧实时
        ...
        # ① 资源监测：每帧只算 1 个 bar（HP/MP/EXP 轮着来）
        _t = time.time()
        ...
        _t = time.time()
        with self._nav_lock:
            self._nav_frame_slot = frame
            player_map = self._nav_latest
        ...
        # ⑥ 预览绘制（保持 10Hz 不变）
        _t = time.time()
        if now - self._last_preview >= 0.20:
    ...
    def _thresholds(self):
        """把阈值配置里的数值转 float（容错字符串写法）

        ★ v24 修复：去掉跨帧缓存。
           之前为省几微秒加了 self._th_cache，但 GUI 的 slider 修改
           cfg 后不会调用 reload_runtime → 缓存永不失效 → 攻击范围
           改不动。现在每次重新读 self.cfg["thresholds"]。

        成本：dict 遍历 + 几十次 float 转换，**几十微秒**，完全可忽略。
        收益：GUI 滑块拖动立刻生效，无需重载/重启。
        """
    ...
    def _draw_patrol(self, ann, player_map):
        ...
        ★ v24 优化：缓存"map + route"的合成结果
           · map_bgr 和 route 只在 reload 时才变
           · 现在每帧都在做 map.copy() + route.max() + panel[nz] = route[nz]
             → 合成一次 5~15ms，30 FPS 下 CPU 浪费巨大
           · 缓存后：合成只做一次，后续每帧只 crop + resize
             → 预览耗时从 19.8ms → 3~5ms
        """
        ...
        # 没玩家位置 → 跳过（提前 return，省去无谓的 crop/resize）
        if player_map is None or len(player_map) < 2:
            return

        # ★ 缓存合成后的 panel（map + route）
        #   缓存失效：只由 reload_runtime() 触发（那里会把缓存置 None）
        panel = getattr(self, "_patrol_panel_cache", None)
        if panel is None:
            panel = map_bgr.copy()
            ...
            if route is not None and route.shape[:2] == panel.shape[:2]:
                ...
                panel[nz] = route[nz]
            self._patrol_panel_cache = panel
