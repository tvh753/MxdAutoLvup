...
class PatrolNavigator:
    ...
    【注意】本模块是 v2 的参考实现，目前引擎已切换为 color_route.py
    （v5 颜色路线导航）。保留此处便于对比学习两代算法差异：
      · v2 按「路点 + 动作」推进：walk/jump/rope_up/rope_down；
      · v5 直接在颜色路线图上做像素级寻路，不再需要手录路点。

    核心机制：
      - 玩家点最近邻追踪（抗 NPC 黄点干扰）+ 闪烁累积兜底；
      - 行走/跳跃到点只判水平（游戏无法竖直微调）；
      - 抓绳：按住 上/下 → 检测 y 变化确认上绳 → 攀爬到目标高度，
        未上绳退开重试，超次跳过该路点。
    """
    def __init__(self):
        # ---- 配置 ----
