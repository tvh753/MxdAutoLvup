...
用户按住左键拖拽框选一块区域，松开后回传原图坐标。
三种模式（mode）决定回传结果的格式：
  template —— 模板截取 → (x, y, w, h)
  bar      —— 状态条校准 → {"x","y","w","h"}（颜色由 BarMonitor 自适配）
  region   —— 检测区域 → [x, y, w, h]
"""
...
class RegionSelector(tk.Toplevel):
    ...
    def __init__(self, master, frame_bgr, mode="template", on_ok=None, tip=None):
        ...
        self.grab_set()  # 模态：锁定焦点，必须先关闭本窗口

        # 按屏幕尺寸等比缩放画面，保证在窗口内看得全
        ...
        img = Image.fromarray(np.ascontiguousarray(frame_bgr[..., ::-1]))  # BGR→RGB
        if self.scale < 1.0:
    ...
    def _ok(self):
        ...
        if self.mode == "bar":  # 只回传区域，颜色由 BarMonitor 自适配
            ...
            result = (x, y, w, h)
        cb, self.on_ok = self.on_ok, None  # 先取回调再销毁，防重入
        self.destroy()
        if cb:
            cb(result)